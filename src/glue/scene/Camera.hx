...
@:final class Camera
{
	...
	var _delayFactor:Float = Constants.DEFAULT_CAMERA_DELAY;

	public function new()
	{
		position = Vector2D.create(Glue.width * Constants.HALF, Glue.height * Constants.HALF);
	}

	public function follow(target:Entity, delayFactor:Float = Constants.DEFAULT_CAMERA_DELAY)
	{
		_target = target;
